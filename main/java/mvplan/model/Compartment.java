...
public class Compartment implements Serializable
{
    ...
    public void setpp(double ppHe, double ppN2) throws ModelStateException
    {
        if(ppHe<0.0 || ppN2 <0.0) {
            if((MvplanInstance.getMvplan().getDebug() > 0)) System.out.println("ERROR: setpp() is throwing exception, ppHe="+ppHe+" ppN2="+ppN2);        
    ...
    public void constDepth(double ppHeInspired, double ppN2Inspired, double segTime) throws ModelStateException
    {
       ...
       if(ppHeInspired<0.0 || ppN2Inspired <0.0 || segTime <0.0){
            if((MvplanInstance.getMvplan().getDebug() > 0)) System.out.println("ERROR: constDepth() is throwing exception, ppHe="+ppHeInspired+" ppN2="+ppN2Inspired);  
    ...
    {        
        if(ppHeInspired<0.0 || ppN2Inspired<0.0 || segTime < 0.0) {
            if((MvplanInstance.getMvplan().getDebug() > 0)) System.out.println("ERROR: ascDec() is throwing exception, ppHe="+ppHeInspired+" ppN2="+ppN2Inspired);  
